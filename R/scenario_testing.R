...
# Server function -----------
  server <- function(input, output, session) {
    ...
    # Assemble Parameters -------
    my_params <- reactive({
      # print("my_params updated")
      # list(
      #   vessels = updatedScenario()[["vessels"]],
      ...
      #   phase = input$phase
      # )
      # print(paste("Current bubble level:", input$bubble))
      list(ambient = input$bufferRadius,
           sourceLvL = input$sl,
           logrange = input$alpha,
           absorb = input$abs_coef,
           lowerdB = input$bubble,
    ...
    # observe({
    #   print(paste("Bubble level changed to:", input$bubble))
    # })
    
    
    # Update Scenario List Object ----- 
    updatedScenario <- eventReactive(input$saveBtn, {
      print("updatedScenario triggered")
      updated_list <- update.narwscenario(selectedList(), currentTable())
    ...
    updatedList <- reactive({
      ...
      modifyList(selectedList(), my_params())
    ...
    output$download <- downloadHandler(
      ...
      content = function(file) {
        current_params <- updatedList()
        save(current_params, file = file)
      }
    )
    ...
    observe({
      ...
      cat("Data captured at: ", Sys.time(), "\n") 
    })
    
    # Exit Handling
    session$onSessionEnded(function() {
      print("Session is ending; saving parameters...")
      if (!is.null(latestData)) {
        print(latestData)
        saveParameters(latestData)
      }
      stopApp()  
    })
