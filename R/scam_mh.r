#' Posterior sampling for shape constrained GAMs
#'
#' Posterior sampling
#' @export
#' @import mgcv
...
#' }
#' 
scam_mh <- function(b, ns = 10000, burn = 1000, t.df = 40, rw.scale = .25, thin = 1, pb = TRUE, verbose = FALSE) {
  ...
  beta <- tsgam:::coef.scam(b) |> as.numeric()
  Vb <- tsgam:::vcov.scam(b)

  # beta <- coef(b)
  # Vb <- vcov(b)

  # X <- model.matrix(b)
  X <- scam:::predict.scam(b, type = "lpmatrix")
  burn <- max(0, burn)
  # prog <- interactive()
  prog <- pb
  iprog <- 0
  di <- floor((ns + burn) / 100)
  if (prog) prg <- txtProgressBar(min = 0, max = ns + burn, initial = 0, char = "=", width = NA, title = "Progress", style = 3)
  ...
  if (rw) {
    R <- chol(Vb)
    step <- mgcv::rmvn(ns + burn, beta * 0, Vb * rw.scale) ## random walk steps (mgcv::rmvn)
  }
  u <- runif(ns + burn)
  ...
  if (prog) setTxtProgressBar(prg, i)
  if(verbose & prog) cat("\n")
  if(verbose) cat("fixed acceptance = ", accept / ns, "  RW acceptance = ", rw.accept / ns, "\n")
