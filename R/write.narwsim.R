...
write.narwsim <- function(obj, ...){
  ...
  for(k in cohort.ab){
    ...
    const <- sim[[k]][whale %in% whaleID, list(day, date, month, whale, cohort)]

    xy <- cbind(const, sim[[k]][whale %in% whaleID, list(easting, northing, region, d_travel, swimspeed)])

    attrib <- cbind(const, sim[[k]][whale %in% whaleID, list(alive, age, starve, bc, length, length_a, length_b, length_c, mass, leanmass, fatmass, mass_a, mass_b, mouth_r, mouth_a, mouth_w, gsl, seus)])
    
    if (cohorts[abb == k, id] == 5) {
      attrib_calf <- cbind(const[, !c("cohort")], sim[[k]][whale %in% whaleID, list(cohort_calf, born, alive_calf, age_calf, starve_calf, bc_calf, length_calf, La_calf, Lb_calf, Lc_calf, mass_calf, leanmass_calf, fatmass_calf, ma_calf, mb_calf, mouth_r_calf, mouth_a_calf, mouth_w_calf)])
    } else {
    ...
    activ <- cbind(const, sim[[k]][whale %in% whaleID, list(t_travel, t_feed, t_nurse, t_rest, t_sum, t_remain)])

    if (cohorts[abb == k, id] == 4) {
      gest <- cbind(const, sim[[k]][whale %in% whaleID, list(abort, fetus_l, fetus_m, delta_fetus_m, delta_fetus_l, birth_l, birth_m, muscle_m, viscera_m, bones_m, blubber_m, muscle_lip, muscle_pro, visc_lip, visc_pro, bone_lip, blubber_lip, blubber_pro)])
    } else {
    ...
    stressors <- cbind(const, sim[[k]][whale %in% whaleID, list(gear_risk, is_entgl, entgl_head, entgl_sev, entgl_d, entgl_start, entgl_end, is_entgl_calf, entgl_head_calf, entgl_sev_calf, entgl_d_calf, entgl_start_calf, entgl_end_calf, strike_risk, strike, strike_calf, noise_resp, noise_lvl, dB_thresh)])

    feed <- cbind(const, sim[[k]][whale %in% whaleID, list(feed, preyconc, minprey, gape, feedspeed, captEff, impedance, daylight, feed_effort, targetBC, cop_mass, cop_kJ, digestEff, metabEff, E_cop)])

    growth <- cbind(const, sim[[k]][whale %in% whaleID, list(delta_fat, ED_lip, lip_anab, lip_catab)])

    energy <- cbind(const, sim[[k]][whale %in% whaleID, list(E_tot, E_in, E_out, rmr, LC, scalar_LC, stroke, E_growth, E_gest, fgrowth, placenta, hic, E_lac, delta_m)])
    
    if (cohorts[abb == k, id] == 5) {
    energy_calf <- cbind(const[, !c("cohort")], sim[[k]][whale %in% whaleID, list(cohort_calf, E_tot_calf, E_in_calf, E_out_calf, rmr_calf, LC_calf, scalar_LC, E_growth_calf, delta_m_calf)])
    } else {
