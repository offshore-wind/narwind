...
::: {style="width:850px"}
![**Figure 2.1.** Locations of wind farm sites included in the default narwind scenarios.](https://github.com/offshore-wind/narwind/blob/main/vignettes/windfarm_sites.jpg)
:::
...
![**Figure 2.2.** Location of the wind farms considered in the Southern New England (SNE) scenarios](https://github.com/offshore-wind/narwind/blob/main/vignettes/BOEM_140M0121C0008_figure34.jpg)
...
![**Figure 2.3.** Location of the wind farms considered in the Virginia (VA) scenarios.](https://github.com/offshore-wind/narwind/blob/main/vignettes/BOEM_140M0121C0008_figure35.jpg)
...
::: {style="width:650px"}
![**Figure 2.4.** Schedule of piling activities under three pre-defined offshore wind scenarios.](https://github.com/offshore-wind/narwind/blob/main/vignettes/BOEM_140M0121C0008_figure40.jpg)
:::
...
![**Figure 2.5.** Layout and structure of the Shiny app within narwind.](https://github.com/offshore-wind/narwind/blob/main/vignettes/shiny_main.png)
...
First, start by clicking on the Scenario Seletion tab. The central text area will change to reveal several drop-down menus (Figure 2.6).
...
To define a custom scenario, choose Custom Scenario. Three new fields will appear, allowing the user to upload files for the wind energy area (WEA) boundary, the piling locations and timings, and the vessel routes that underpin the scenario they wish to explore. An additional drop-down menu will also be available to indicate whether the custom scenario relates to construction or operational activities. This is important, as it ensures that the scenario object is correctly labeled. The custom scenario can then be viewed in the same way as the pre-loaded scenarios, and scenario parameters can be modified using the controls found in the Piling Noise and Vessel Traffic tabs. As with the pre-loaded scenarios, modifications made to scenario parameters within the app tabs will feed through to the final R object that will be used in the bioenergetic model.

![**Figure 2.6.** Scenario set up tab.](https://github.com/offshore-wind/narwind/blob/main/vignettes/shiny_setup.png)
...
![**Figure 2.7.** List of turbine locations.](https://github.com/offshore-wind/narwind/blob/main/vignettes/shiny_csv.jpg){width="500"}

Similarly, vessel routes to each wind farm site must be provided as a `.shp` file of spatial line features, each with the following attributes: a `windfarm` ID (same as above) and a sequential `routeID` number (`1`, `2`, etc.) for each route associated with a particular farm. A `.shp` file must also be provided for the wind energy area (WEA) boundary.
...
Ticking the `Vessel density` checkbox prompts the user to choose whether they wish to display a baseline vessel density layer or a scenario vessel density layer (relating to the scenario selected on the Scenario Selection tab). The user should also select which month they wish to visualize. The colour scale goes from yellow (low density) through orange to green/blue, with green/blue indicating the highest vessel density.
The table on this tab allows the user to specify the number, speed and number of trips made by each class of vessel that should be included in the scenario parameters for the bioenergetic model. Values in the table can be changed by clicking on the relevant cell. Changes can be saved by clicking the `Save Changes` button at the bottom.

### Whale density

Ticking the `Whale density` checkbox overlays whale density on the map according to the month that is selected. The colour scale goes from dark blue (low density) through green to yellow, with yellow indicating the highest whale density areas.

### Download

Once all changes to scenario parameters have been made, the scenario object can be downloaded using the green `Download Scenario` button located on the left. The saved scenario will also be automatically loaded into the open R session, for use in the bioenergetic model. 

### Geographical features

The bottom of the left-hand side-bar consists of a list of check boxes that allow different geographical features to be visualized on the map. This includes the boundaries of the pre-loaded wind farm sites, the vessel routes to these wind energy areas, the turbine locations, as well as the location of traffic separation schemes, areas to be avoided (for NARW), and NARW speed zones.  
