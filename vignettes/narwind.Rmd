...
The bioenergetic model is written in the R and C++ programming languages. To run the model, users need to install the freely available [R software](https://cloud.r-project.org/) on their machines. We also recommend the use of the open-source [RStudio software](https://posit.co/download/rstudio-desktop/), which provides a user-friendly front-end interface to R, with many convenient features and tools.

Detailed installation instructions are available in many introductory tutorials, including:

-   [Getting Started with Data in R](https://moderndive.netlify.app/1-getting-started) (Chapter 1 in the *Statistical Inference via Data Science: A ModernDive into R and the Tidyverse*! book by Chester Ismay and Albert Y. Kim, 2023).

-   [Getting started with R and RStudio](https://ourcodingclub.github.io/tutorials/intro-to-r/) from the Coding Club (article by Gergana Daskalova).

-   [RStudio tutorial](https://www.datacamp.com/tutorial/r-studio-tutorial) from the DataCamp blog (article by Elena Kosourova).

-   [Basic Basics RYouWithMe Unit 1](https://rladiessydney.org/courses/ryouwithme/01-basicbasics-0/) from the R-Ladies Sydney group.
