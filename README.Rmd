...
If you are just getting started with `narwind`, we recommend reading the tutorial vignette (under `Articles > Getting started`), which provides a detailed introduction to the package.
...
The package relies on compiled code (C++) and functionality provided by the [Rcpp](https://cran.r-project.org/web/packages/Rcpp/index.html) package. The `Rtools` software may be needed on Windows machines. Installation instructions can be found at <https://cran.r-project.org/bin/windows/Rtools/rtools40.html>.
